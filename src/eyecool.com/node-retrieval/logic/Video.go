...
func (VideoLogic) UpdateVideo(video *model.Video) error {
	...
	defer session.Close()
	session.Begin()
	_, err := MasterDB.Where("pk_id=?", video.PkId).Cols("extra_meta","rec_params","name","url","enabled").Update(video)
	if err != nil {
		session.Rollback()
...
func (VideoLogic) DeleteVideo(video *model.Video) error {
	...
	if err != nil {
		session.Rollback()
		log.Println("delete video error:", err)
		session.Commit()
		return err
	}
	session.Commit()
...
func (videoLogic VideoLogic) FindVideoByPkId(pkId int) (bool, *model.Video) {
	video := new(model.Video)
	has, err := MasterDB.Table(video.TableName()).Where("pk_id = ?", pkId).Get(video)
	if err != nil {
		log.Println("FindVideoByPkId error : ", err)
	}
	return has, video
}
