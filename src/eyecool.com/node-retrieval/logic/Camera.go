...
func (CameraLogic)CameraQuery() []*Cameras{
	...
	for _,v:=range cameras{
		...
		if l:=len(v.PredecessorId);l>0{
			...
			camera.PredecessorIds=DefaultCamera.GetRegionList(id)
		}
...
func (CameraLogic) SelectCameraById(id int)(bool,*model.Camera){
	objLog := GetLogger(nil)
	camera:=model.Camera{}
	flag,err:=MasterDB.Where("id=? and status!=6",id).Get(&camera)
	if err != nil {
		objLog.Errorln("CameraLogic selectCameraById PageAd error:", err)
		return false,nil
	}
	return flag,&camera
}

func  (CameraLogic) GetRegionList(id int) []string {
	//objLog := GetLogger(nil)
...
func (CameraLogic)Update(camera *model.Camera)error{
	...
	if err != nil {
		session.Rollback()
		logger.Errorln("update camera error:", err)
		return  err
	}
	session.Commit()
	return nil
...
func (CameraLogic)InsertVideoCamera(vc *model.VideoCamera) error {
	...
	if err != nil {
		fmt.Println(err)
		session.Rollback()
		logger.Errorln("insert videoCamera error:", err)
		return  err
	}
	session.Commit()
	return nil
