...
//插入摄像机
func InsertCamera(camera *CameraRequest) *InsertCameraResponse {
	result := judgeCameraRequestParams(camera)
	if result.Rtn != 0 {
		return result
	}
...
func judgeCameraRequestParams(request *CameraRequest) *InsertCameraResponse {
	...
	if request.PredecessorId == "" {
		result.Rtn = -1
		result.Message = "所属组不能为空！"
		return result
	}
	if request.Ip == "" {
		result.Rtn = -1
		result.Message = "Ip不能为空！"
		return result
	}
	return result
}
